...
/* ====== Protótipos para resolver ordem de chamadas ====== */
...
static void check_function(Node *fn, int *errors) {
    ...
    for (size_t i = 0; i < fn->u.as_function.param_count; i++) {
        Node *pd = fn->u.as_function.params[i]; // ND_DECL
